...
Bzf=0.00
h1=0.01     	    ; step size?
string1='derivs2_5' ; km: This needs to match with string1 in CMTODEINT
   ...
   endelse
    ...
    IF pno eq 0 THEN BEGIN
     IF number eq 0 THEN BEGIN
       ...
       print, maxb
     ENDIF
     myplot=plot(xf, yf, xrange=[-2,2], yrange=[0,6.5],POSITION=[0.22,0.1,0.95,0.85],$; COLOR=flinecol, $
     ztitle='y (10Mm)', xtitle='x (10Mm)',ytitle='z (10Mm)' $
     ,RGB_TABLE=ctable, VERT_COLORS=BYTSCL(bb)*max(bb)/maxb $ ;33
     )
     col=COLORBAR(RANGE=[0,maxb], orientation=1, POSITION=[0.08,0.1,0.13,0.85],TITLE='field strength (0.01T)')
     if particleson eq 1 then begin
